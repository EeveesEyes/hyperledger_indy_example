...
class AnoncredsRevocation {
	...
	static void demo() throws Exception {
		...
		/* enable issuer to write to the Ledger (this should be proxied by the steward) */
		String seed = "000000000000000000000000Steward1";
		String did_json = new JSONObject().put("seed", seed).toString();
		DidResults.CreateAndStoreMyDidResult issuerDidResult = createAndStoreMyDid(issuerWallet, did_json).get();
		String issuerDid = issuerDidResult.getDid();
		...
		/* Write Schema to the Ledger */
		String schemaReq = Ledger.buildSchemaRequest(issuerDid, schemaJson).get();
		// Writing schema req to the ledger
		String schemaRes = Ledger.signAndSubmitRequest(pool, issuerWallet, issuerDid, schemaReq).get();
		JSONObject jsonSchemaRes = new JSONObject(schemaRes);
		int schemaSeqNo = jsonSchemaRes.getJSONObject("result").getJSONObject("txnMetadata").getInt("seqNo");

		/* Write CredDef to the Ledger */
		JSONObject jsonCredDefReq = new JSONObject(Ledger.buildCredDefRequest(issuerDid, credDefJson).get());
		jsonCredDefReq.getJSONObject("operation").put("ref", schemaSeqNo); // add the correct schema transaction number to the request
		String credDefRes = Ledger.signAndSubmitRequest(pool, issuerWallet, issuerDid, jsonCredDefReq.toString()).get();
		JSONObject jsonCredDefRes = new JSONObject(credDefRes);
		int credDefSeqNo = jsonCredDefRes.getJSONObject("result").getJSONObject("txnMetadata").getInt("seqNo");

		/* Write RevRegDef to the Ledger */
		JSONObject jsonRevocRegDefReq = new JSONObject(Ledger.buildRevocRegDefRequest(issuerDid, revRegDefJson).get());
		jsonRevocRegDefReq.getJSONObject("operation").put("ref", credDefSeqNo);// add the correct credDef transaction number to the request
		String revocRegDefReq = jsonRevocRegDefReq.toString();
		String revocRegDefRes = Ledger.signAndSubmitRequest(pool, issuerWallet, issuerDid, revocRegDefReq).get();
		JSONObject jsonRevocRegDefRes = new JSONObject(revocRegDefRes);
